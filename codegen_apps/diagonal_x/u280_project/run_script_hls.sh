...
# Parameter tuple format for 2D apps: "sizex,sizey,iters,batch"
...
for params in "${parameter_sets[@]}"; do
    ...
    if [[ "${CXXFLAGS}" == *"-DPOWER_PROFILE"* ]]; then
        ...
        ${OPS_INSTALL_PATH}/../scripts/power_profile_hls.sh ${DEVICE_BDF} \
            ${SCRIPT_DIR}/hls/build/${TARGET_MODE}/${APP_NAME}_host \
            ${SCRIPT_DIR}/hls/build/${TARGET_MODE}/${APP_NAME}.xclbin \
            -sizex="${sizex}" -sizey="${sizey}" -iters="${iters}" -piter="${batch}"
    else
        if [[ ${TARGET_MODE} == sw_emu || ${TARGET_MODE} == hw_emu ]]; then
            echo "Running in emulation mode with ${TARGET_MODE}"
            XCL_EMULATION_MODE=${TARGET_MODE} \
                ${SCRIPT_DIR}/hls/build/${TARGET_MODE}/${APP_NAME}_host \
                ${SCRIPT_DIR}/hls/build/${TARGET_MODE}/${APP_NAME}.xclbin \
                -sizex="${sizex}" -sizey="${sizey}" -iters="${iters}" -batch="${batch}"
        else
